...
만약 일부 질문에 답변이 없더라도, 억지로 내용을 만들어내지 말고
제공된 답변을 중심으로 자연스럽게 구성하세요.
...
1월 한 줄 회고
"위에서 작성한 에세이 전체 내용을 바탕으로,
그 달을 압축해 표현한 정리된 한 문장"

위 형식을 지키지 않으면 글은 완성되지 않은 것입니다.
`

  const userPrompt = `다음은 사용자의 2026년 1월 회고 답변입니다. 이 답변들을 바탕으로 따뜻한 회고 에세이를 작성해주세요:
  아래 답변 중 비어 있는 항목은 무리해서 해석하거나 추측하지 말고,
  작성된 내용 위주로만 글을 구성해주세요.
1. 1월을 한 문장으로: ${answers.q1 || ""}
2. 이 달의 전반적인 감정: ${answers.q2 || ""} ${answers.q2Reason ? `(이유: ${answers.q2Reason})` : ""}
3. 가장 기억에 남는 장면: ${answers.q3 || ""}
4. 그 일이 기억에 남는 이유: ${answers.q4 || ""}
5. 이번 달의 나는 어떤 사람이었나요: ${answers.q5 || ""}
6. 잘한 선택: ${answers.q6 || ""}
7. 아쉬운 선택: ${answers.q7 || ""}
8. 가장 많이 떠오른 사람: ${answers.q8 || ""}
9. 그 사람과 관련된 감정: ${answers.q9 || ""}
10. 일상 키워드: ${[answers.q10_1, answers.q10_2, answers.q10_3].filter(Boolean).join(", ")}
11. 가장 에너지를 쓴 곳: ${answers.q11 || ""}
12. 1월을 지나며 알게 된 나에 대한 사실 한 가지: ${answers.q12 || ""}
13. 2월의 나에게, 미리 해주고 싶은 말 한마디: ${answers.q13 || ""}`
